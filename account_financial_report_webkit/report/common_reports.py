...
SELECT account_move.id,
       array_to_string(
          ARRAY(SELECT DISTINCT a.code
                ...
                AND m2.account_id<>account_move_line.account_id
                ORDER BY a.code), ','), '|',
        array_to_string(
          ARRAY(SELECT sum(m2.debit + m2.credit)
                FROM account_move_line m2
                  LEFT JOIN account_account a ON (m2.account_id=a.id)
                WHERE m2.move_id =account_move_line.move_id
                AND m2.account_id<>account_move_line.account_id
                GROUP BY m2.move_id, a.code
                ORDER BY m2.move_id, a.code), ',')
FROM account_move
        JOIN account_move_line
            on (account_move_line.move_id = account_move.id)
        JOIN account_account
            on (account_move_line.account_id = account_account.id)
        WHERE account_account.id = %s AND move_id in %s
        GROUP BY account_move.id, account_account.code, account_move_line.id
        ORDER BY account_move_line.id
        """

        try:
            self.cursor.execute(sql, (account_id, tuple(move_ids)))
        ...
        ret = {}
        for move in res:
            contra = [x for x in list(move[1:])]
            ret.update({move[0]: contra})
            #_logger.info("Contra %s" % list(move[1:]))
        return ret
