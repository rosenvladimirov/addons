...
class GeneralLedgerWebkit(report_sxw.rml_parse, CommonReportHeaderWebkit):
    ...
    def _get_ledger_lines(self, move_line_ids, account_id):
        ...
        for line in res:
            listing = counter_parts.get(line.get('move_id'), [])
            for account,x in enumerate(listing):
                if x == '|':
                    break
            contra_acc = listing[:account][0].split(',')
            contra_money = listing[account+1:][0].split(',')
            #_logger.info("Contra %s:%s:%s" % (contra_acc, contra_money, account))
            acc = []
            for i,x in enumerate(contra_acc):
                #_logger.info("contra item %s:%s:%s" % (contra_acc[i], contra_money[i], i))
                acc.append([contra_acc[i].encode('utf8'), contra_money[i].encode('utf8')])
            line['counterparts'] = acc
        return res
