...
class ProductProduct(models.Model):
    ...
    def search(self, domain, *args, **kwargs):
        global domain_old_supp
        if filter(lambda x: x[0] == 'seller_ids.product_code', domain):
            _logger.info("Start Filter %s:%s" % (domain, domain_old_supp))
            counter = len(filter(lambda x: x[0] == 'seller_ids.product_code', domain))
            if counter > 1:
                j = 0
                for i,x in enumerate(domain):
                    if x[0] == 'seller_ids.product_code' and j < counter-1:
                        #_logger.info("domain %s:%s:%s" % (x[0], i, j))
                        domain[i] = domain_old_supp[j]
                        j += 1
            else:
                domain_old_supp = []
            supp_code_operator = filter(lambda x: x[0] == 'seller_ids.product_code', domain)[0][1]
            supp_code_value = filter(lambda x: x[0] == 'seller_ids.product_code', domain)[0][2]
