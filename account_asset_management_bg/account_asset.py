...
class account_asset_category(orm.Model):
    ...
    _columns = {
        ...
        'method_number': fields.integer('Number of Years', help="The number of years needed to depreciate your asset"),
        ...
        'type': fields.selection([
            ('normal', 'For accounting '),
            ('taxadmin', 'From Tax administration'),
            ], 'Type',
            readonly=True),
    }
    ...
    _defaults = {
        ...
        "type": lambda self,cr,uid,c:c.get('mode','normal'),
    }
    ...
    _constraints = [(_check_method, "Degressive-Linear is only supported for Time Method = Year.", ['method']), 
                    ]
    ...
    def onchange_method_time(self, cr, uid, ids,
        ...
        if method_time == 'number':
            res['value'].update({'method_period': 'month'})
        return res
...
class account_asset_asset(orm.Model):
    ...
    def _get_depreciation_stop_date(self, cr, uid, asset,
        ...
        elif asset.method_time == 'percentage':
                depreciation_stop_date = depreciation_start_date + \
                    relativedelta(months=int((100.0/asset.method_percentage)*12), days=-1)
    ...
    def _compute_year_amount(self, cr, uid, asset, amount_to_depr,
        ...
        if asset.method_time == 'year':
            ...
            divisor = 100.0/asset.method_percentage
    ...
    _columns = {
        ...
        'category_id': fields.many2one(
            ...
            domain=[('type', '=', 'normal')],
            states={'draft': [('readonly', False)]}),
        'taxadmin_category_id': fields.many2one(
            'account.asset.category', 'Tax Admin Asset Category',
            change_default=True, readonly=True,
        ...
        'method': fields.selection(
            ...
            help="Choose the method to use to compute "
                 ...
                 "depreciation exceeds the annual degressive depreciation"),
        'method_percentage': fields.float('Depreciation percentage', digits=(3, 2)),
        'method_taxadmin_percentage': fields.related('taxadmin_category_id','method_percentage', type='float', string='Tax admin depreciation percentage',
            help="This is legal defined by low depreciation percentage."),
        'method_number': fields.integer(
            'Number of Periods', readonly=True,
            states={'draft': [('readonly', False)]},
            help="The number of periads like years, months needed to depreciate your asset"),
        'method_period': fields.selection([
            ('month', 'Month'),
            ('quarter', 'Quarter'),
            ('year', 'Year'),
    ...
    def onchange_category_id(self, cr, uid, ids, category_id, context=None):
        ...
        if category_id:
            ...
            res['value'] = {
                ...
                'method_percentage': category_obj.method_percentage,
            }
        return res
    ...
    def onchange_method_percentage(self, cr, uid, ids,
                             method_percentage, method_period, method_time='percentage', context=None):
        res = {'value': {}}
        if method_time == 'percentage' and method_period == 'month':
            res['value'] = {'method_number': int(100.0/method_percentage * 12)}
        return res
...
class account_asset_depreciation_line(orm.Model):
    ...
    _columns = {
        ...
        'type': fields.selection([
            ...
            ('taxadmin', 'Depreciation defined by Tax administaration'),
            ], 'Type', readonly=True),
    ...
    _defaults = {
        "type": lambda self,cr,uid,c:c.get('mode','depreciate'),
        #'type': 'depreciate',
    }
