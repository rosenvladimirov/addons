    ...
    def _average_consumption_history(self):
        for product in self:
            ...
            history_ids = self.env['product.history'].search([
                ...
                ('ignored', '=', 0)]).sorted()
            _logger.info("History %s:%s" % (nb, history_ids))
            ...
            if nb == 0:
                ...
                _logger.info("History end %s:%s" % (nb, total_consumption))
...
# Action section
    ...
    def _compute_history(self, history_range):
        #_logger.info('Start compute history %s' % history_range)
        now = date.today()
        for product in self:
            ...
            if history_ids:
                ...
                last_qty = 0
            #_logger.info('Calculate history %s:%s:%s:%s' % (from_date + delta - td(days=1), from_date + rd(days=-1), now, from_date + delta, from_date))
            while from_date + delta <= now:
                ...
                #_logger.info("Take movments %s:%s:%s" % (res, res2, res3))
                vals = {
                    'product_id': product.id,
