        ...
        for line in self.line_ids:
            if line.purchase_qty != 0:
                line_values = {
                    ...
                    'taxes_id': [[6, 0, fiscal_obj.map_tax(self._cr, self._uid, self.fiscal_position, line.product_id.taxes_id, context=self._context)]],
        ...
        for cpo in self:
            ...
            for psi in psi_obj.search([('name', '=', cpo.partner_id.id)]):
                for pp in psi.product_tmpl_id.filtered(
                        lambda pt: pt.state not in ('end', 'obsolete') and pt.purchase_ok
                ).product_variant_ids:
