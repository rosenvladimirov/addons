...
class StockQuant(models.Model):
    ...
    def _account_entry_move_ex(self, cr, uid, parts, context=None):
        ...
        _logger.debug("Averashe standart price start %s:%s:%s=>%s:%s" % (parts['move'].product_id.product_tmpl_id.seller_ids,parts['location_from'].partner_id, parts['company_to'], parts['move'].location_id.usage, parts['move'].location_dest_id.usage))
        ...
        if company_from and company_to and (move.location_id.usage not in ('internal', 'transit', 'pos') and move.location_dest_id.usage in ('pos', 'internal') or company_from != company_to):
        #if move.location_id.usage in ('supplier') and move.location_dest_id.usage in ('pos', 'internal'):
            ...
            _logger.debug("Averashe standart price get %s:%s:%s" % (company_from, company_to, s_ids))
