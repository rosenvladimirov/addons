...
function openerp_pos_screens(instance, module){ //module is instance.point_of_sale
    ...
    module.ScreenSelector = instance.web.Class.extend({
        ...
        show_popup: function(name,options){
            console.log("pop up", name, this.popup_set, this.current_popup);
            if(this.current_popup){
                this.close_popup();
            }
            this.current_popup = this.popup_set[name];
            console.log("seted pop", name, this.current_popup)
    ...
    module.ScreenWidget = module.PosBaseWidget.extend({
        ...
        show: function(){
            ...
            this.pos_widget.set_topheader_visible(this.show_topheader);
            this.pos_widget.set_users_topheader_visible(this.show_users_topheader);
    ...
    module.ScaleScreenWidget = module.ScreenWidget.extend({
        ...
        order_product: function(){
            ...
            selectOrder.addProduct(this.get_product(), {price: this.pos.pricelist_engine.update_products_prices(this.get_product(), selectOrder.get_client()).price, quantity: this.weight, merge: true });
        },
    ...
    module.ProductScreenWidget = module.ScreenWidget.extend({
        ...
        init: function(parent, options){
            this._super(parent, options);
            this.noHeader = _.isEmpty(options) ? false : options.noHeader;
            this.emptyList = _.isEmpty(options) ? false : options.emptyList;
            this.infoHeader = _.isEmpty(options) ? false : options.infoHeader;
            console.log("PrScreenwG", options);
        },
        start: function(){ //FIXME this should work as renderElement... but then the categories aren't properly set. explore why
            var self = this;
            //console.log("Start PL", this.emptyList);
            this.product_list_widget = new module.ProductListWidget(this,{
                click_product_action: function(product){
                    if(product.to_weight && self.pos.config.iface_electronic_scale){
                        ...
                        if(selectOrder.get_product_filter().length !== 0){
                            price = self.pos.taxes_engine.get_all_prices({quantity: 1, unit_price: product.price, disc: 0, force_tax_included: false, product: product,}).priceWithTax;
                            qty = -1;
                        } else {
                            price = self.pos.pricelist_engine.update_products_prices(product, selectOrder.get_client()).price;
                        }
                        selectOrder.addProduct(product, {quantity: qty, price: price, merge: true,});
                    }
                },
                product_list: this.emptyList ? [] : this.pos.db.get_product_by_category(0),
            });
            ...
            if(this.infoHeader) {
                this.product_list_widget.product_info_widget = new module.ProductListInfoWidget(this, {});
                this.product_list_widget.product_info_widget.replace(this.$('.placeholder-ProductCategoriesWidget'));
            }
        },
        show: function(){
            this._super();
    ...
    module.ClientListScreenWidget = module.ScreenWidget.extend({
        ...
        save_changes: function(){
            if( this.has_client_changed() ){
                ...
                currentOrder.set_client(this.new_client);
                console.log("save", this);
                this.pos.pricelist_engine.update_products_ui(this.new_client);
                this.pos.pricelist_engine.update_ticket(this.new_client, orderLines);
            }
        },
        ...
        save_client_details: function(partner) {
            ...
            if (!fields.name) {
                ...
                //});
                // rosen customer-is-required
                this.pos_widget.screen_messages.errors('customer-is-required');
                return;
            }
            ...
            new instance.web.Model('res.partner').call('create_from_ui',[fields]).then(function(partner_id){
                ...
                //});
                // rosen connection-is-down
                this.pos_widget.screen_messages.errors('connection-is-down');
            });
        },
        ...
        load_image_file: function(file, callback){
            ...
            if (!file.type.match(/image.*/)) {
                ...
                //});
                this.pos_widget.screen_messages.errors('unsupported-file-format');
                return;
                // rosen unsupported-file-format
            }
            ...
            reader.onerror = function(){
                ...
                //});
                this.pos_widget.screen_messages.errors('read-image');
                // rosen read-image
            };
    ...
    */
    module.OrderListScreenWidget = module.ScreenWidget.extend({
        ...
        start: function() {
            ...
            this.$el.find('.button.refund').click(function(){
                self.on_click_refund_order();
            });
            this.$el.find('.button.pay').click(function(){
                self.on_click_pay_order();
            });
            this.$el.find('.button.add').click(function(){
                self.on_click_plus_order();
            });
            this.$el.find('.button.remove').click(function(){
                self.on_click_minus_order();
            });
            this.$el.find('.button.merge').click(function(){
                self.on_click_merge_order();
            });
            this.$el.find('.button.reset').click(function(){
                order = self.pos.get('selectedOrder');
                self.reset_order(order);
                self.pos_widget.order_widget.change_selected_order();
            });
        ...
        add_product_attribute: function(product, key, orderline){
            console.log("Attribute", product, key, orderline);
            return product;
        },

        load_order_fields: function(order, fields, options) {
            var order_ids = order.get_order_ids() || [];
            var partner = this.pos.db.get_partner_by_id(
                fields.partner_id);
            order_ids.push(fields.id);
            order.set_order_id(fields.id);
            order.set_order_ids(order_ids);
            order.set_client(partner || undefined);
            order.set_order_state(options.mode);
            order.setName(fields.name);
            return order;
        },

        prepare_orderline_options: function(orderline, options, order_id){
            var qty = orderline.qty;
            if(orderline.type === 'money' || options.mode === 'refund'){
                qty = -qty;
            }
            return {
                quantity: qty,
                price: {price: orderline.price_unit, tax_included: false},
        ...
        load_order_db: function(order_id, order, options) {
            var self = this;
            var orderModel = new instance.web.Model(this.model);
            options = options || {mode: 'standart', merge: false};
            //console.log("load order from db", options);
            return orderModel.call('load_order', [order_id])
            .then(function (result) {
                var orderlines = result.orderlines || [];
                var unknown_products = [];
                order = self.load_order_fields(order, result, options);
                for (var i=0, len=orderlines.length; i<len; i++) {
                    ...
                    order.addProduct(product,
                                 self.prepare_orderline_options(orderline, options, result.id));
                    //last_orderline = order.getLastOrderline();
                    //last_orderline = jQuery.extend(last_orderline, orderline);
                    if(options.mode === 'refund') {
                        product.order_price = orderline.price_unit;
                        product.max_qty = orderline.qty;
                        product_filtered.push(product);
                    }
                }
        ...
        load_order: function(order_id, mode) {
            var order = this.pos.get('selectedOrder');
            var modes = ['standart', 'merge','refund'];
            var order_ids = [];
            var merge_mod = false;
            var fLen = 0;
            this.break_add = false;
            order.set_order_state(mode);
            ...
            fLen = order_ids.length;
            console.log("load order", order_ids);
            for (i = 0; i < fLen; i++) {
                this.load_order_db(order_ids[i], order, {mode: mode, merge: merge_mod});
                if(this.break_add){
                    break;
                } else {
                    //this.pos_widget.action_bar.set_button_disabled('print',false);
                }
            }
        ...
        load_orders: function(query) {
            var self = this;
            var orderModel = new instance.web.Model(this.model);
            self.pos.set({synch: { state:'connecting', pending: 10 }})
        ...
        print_button: function() {
            ...
            //this.pos_widget.action_bar.set_button_disabled('print',true);
        },
        print: function() {
            this.pos_widget.action_bar.destroy_buttons();
            var receiptScreen = this.pos_widget.screen_selector.screen_set['receipt'];
            this.pos_widget.screen_selector.screen_set['receipt'].finishOrder = function(){
                console.log("destroy", this);
                var ss = this.pos.pos_widget.screen_selector;
                ss.set_current_screen('orderlist');
                ss.screen_set['orderlist'].print_button();
            }
        ...
        on_click_merge_order: function(event){
            ...
            this.$el.find('.button.validate').hide();
            $('.ac_selected_order').each(function(idx,el){
                if(el.checked){
                    console.log("Item", parseInt(el.name))
                    order_ids.push(parseInt(el.name));
                }
            });
            console.log("Orders selected", order_ids);
            if(order_ids.length > 0) {
                if( !this.pos.get('selectedOrder').is_empty() ){
                    var order = this.pos.get('selectedOrder');
                    this.reset_order(order);
                }
                this.load_order(order_ids, 'merge');
                if(this.break_add !== true){
        ...
        on_click_draft_order: function(event){
            ...
            this.load_order(this.current_order_id, 'standart');
            if(this.break_add !== true){
                this.$el.find('.button.validate').show();
            }
        },

        on_click_refund_order: function(event){
            product_filtered = [];
            var order_ids = [];
            this.$el.find('.button.next').hide();
            this.$el.find('.button.validate').hide();
            var order = this.pos.get('selectedOrder');
            order_ids.push(order.get_order_id());
            console.log("Orders selected", order_ids);
            if(order_ids.length > 0) {
                this.load_order(order_ids, 'refund');
                if(this.break_add !== true){
                    order.set_product_filter(product_filtered);
                    this.$el.find('.button.next').show();
                }
            }
        },

        on_click_pay_order: function(event){
            product_filtered = [];
            this.$el.find('.button.next').hide();
            this.$el.find('.button.validate').hide();
            console.log('pay', this.pos.config.product_id);
        ...
        on_click_plus_order: function(event){
            product_filtered = [];
            var order_ids = [];
            this.$el.find('.button.next').hide();
            this.$el.find('.button.validate').hide();
            ...
            console.log("Orders selected", order_ids);
            if(order_ids.length > 0) {
                this.load_order(order_ids, 'plus');
                if(this.break_add !== true){
                    this.$el.find('.button.next').show();
                }
            }
        ...
        on_click_minus_order: function(event){
            product_filtered = [];
            var self = this;
            var order_ids = [];
            ...
            if(order_ids.length > 0) {
                ...
                for (var i=0, len=order_ids.length; i<len; i++) {
                    var orderLines = order.getOrderline_by_order_id(order_ids[i]);
                    console.log('Orders get lines', order, orderLines)
                    for (var j=0, lenLine=orderLines.length; j<lenLine; j++) {
                        order.removeOrderline(orderLines[j]);
                    }
                }
            }
        },
        on_click_next_order: function(event){
            var self = this;
            var current_order = this.pos.get('selectedOrder');
            var ss = this.pos.pos_widget.screen_selector;
            if(['standart', 'merge', 'new'].indexOf(current_order.get_order_state()) === -1){
                current_order.set_order_id(undefined);
            }
            this.pos_widget.order_widget.change_selected_order();

            if(current_order.get_order_state() === 'refund'){
                current_order.get('orderLines').reset();
                this.pos_widget.order_widget.change_selected_order();
                console.log("Next", current_order.get_product_filter(), this.pos.pos_widget.product_filtered_screen);
                this.pos.pos_widget.product_filtered_screen.product_list_widget.set_product_list(current_order.get_product_filter());
                this.pos.pos_widget.product_filtered_screen.product_list_widget.set_custom_header(current_order.getName(),
                                                 this.pos.pos_widget.format_currency(current_order.get_order_state(), 'Product Price') + '/' +
                                                 this.pos.pos_widget.format_currency(current_order.getTotalTaxExcluded(), 'Product Price') + '/' +
                                                 this.pos.pos_widget.format_currency(current_order.getTax(), 'Product Price') + '/' +
                                                 this.pos.pos_widget.format_currency(current_order.getTotalTaxIncluded(), 'Product Price'));
                ss.set_current_screen('filtered_products');
            } else {
                ss.set_current_screen('products');
            }
        },
    ...
    module.PaymentScreenWidget = module.ScreenWidget.extend({
        ...
        update_payment_summary: function() {
            ...
            var advTotal = currentOrder.getPaidAdvance();
            var remaining = dueTotal > paidTotal ? dueTotal - paidTotal : 0;
            var change = paidTotal > dueTotal ? paidTotal - advTotal - dueTotal : 0;

            this.$('.payment-due-total').html(this.format_currency(dueTotal));
            this.$('.payment-paid-total').html(this.format_currency(paidTotal));
            this.$('.payment-remaining').html(this.format_currency(remaining));
            this.$('.payment-advance').html(this.format_currency(advTotal));
        ...
        validate_order: function(options) {
            ...
            if(currentOrder.get('orderLines').models.length === 0){
                this.pos_widget.screen_messages.errors('no-lines');
                return;
            }
            ...
            for (var i = 0; i < plines.length; i++) {
                if (plines[i].get_type() === 'bank' && plines[i].get_amount() < 0) {
                    this.pos_widget.screen_messages.errors('negative-bank-payment');
                    return;
                }
            }

            if(currentOrder.getPaidTotal([true]) !== 0 && !currentOrder.get_client()){
                this.pos_widget.screen_messages.errors('no-customer-advance');
                return;
            }
            ...
            if (Math.abs(currentOrder.getTotalTaxIncluded() - currentOrder.getPaidTotal()) > 0.00001) {
                ...
                if (!cash) {
                    this.pos_widget.screen_messages.errors('no-rest-without-cash');
                    return;
                }
            }
            ...
            // Check for buy on credit (take stock loan)
            if(currentOrder.getTotalTaxIncluded() - currentOrder.getPaidTotal() > 0.00001){
                if (this.pos_widget.screen_messages.confirms('loan')) {
                            if(currentOrder.getPaidTotal([true]) === 0){
                                this.pos_widget.screen_messages.errors('no-advance-payment');
                                return;
                            }
                            if(this.pos_widget.screen_messages.confirms('confirm')){
            ...
            if(options.invoice){
                ...
                invoiced.fail(function(error){
                    if(error === 'error-no-client'){
                        ...
                        //});
                        this.pos_widget.screen_messages.errors('anonymous-order');
                    }else{
                        //self.pos_widget.screen_selector.show_popup('error',{
                        //    message: _t('The order could not be sent'),
                        //    comment: _t('Check your internet connection and try again.'),
                        //});
                        this.pos_widget.screen_messages.errors('order-send');
                    }
                ...
                if(this.pos.config.iface_print_via_proxy || this.pos.config.iface_fprint_via_proxy){
                    var receipt = currentOrder.export_for_printing();
                    this.pos.config.printers.forEach(function (printer){
                        if (printer[2]) {
    ...
    module.UsersSelectionScreenWidget = module.ScreenWidget.extend({
        template: 'UsersSelectionScreenWidget',
        ...
        init: function(parent, options){
                ...
                this.notification.appendTo(this.pos_widget.$('.users-rightheader'));

                if(this.pos.config.use_proxy){
                    this.proxy_status = new module.ProxyStatusWidget(this,{});
                    this.proxy_status.appendTo(this.pos_widget.$('.users-rightheader'));
                }
                ...
                this.close_button.appendTo(this.pos_widget.$('.users-rightheader'));
        },
        show_leftpane: false,
        show_topheader: false,
        show_users_topheader: true,

        show: function() {
                this._super();
                var self = this;

                this.$('.user-item').on('click', function(e) {
                        e.stopPropagation();
                        self.select_user(this);
                });
        ...
        check_user_password: function(user_id, password) {
                var self = this,
                input_password = self.pos.db.get_user_password(user_id);
        ...
        select_user_by_id: function(user_id) {
                var self = this;
                this.pos.cashier = this.pos.db.get_user(user_id);
                this.pos_widget.username.refresh();
