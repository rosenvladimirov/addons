...
class pos_session(osv.osv):
    ...
    def create(self, cr, uid, values, context=None):
        ...
        if draftOrder_ids:
            pos_order_obj.write(cr, uid, draftOrder_ids, {'session_id': res.id}, context=context)
        return res
        ...
        for record in self.browse(cr, uid, ids, context=context):
            for st in record.statement_ids:
                if abs(st.difference) > st.journal_id.amount_authorized_diff:
                    ...
                    if not self.pool.get('ir.model.access').check_groups(cr, uid, "point_of_sale_bg.group_pos_manager"):
                        raise osv.except_osv( _('Error!'),
...
class pos_order(osv.osv):
    ...
    def _process_draft_order(self, cr, uid, orders, context=None):
        ...
        for order_tmp in orders:
            ...
            try:
                ...
                fields.update(order_id=order_id)
            if order_id:
                # write
            ...
            for statement_data in statements_data:
                self.add_payment(cr, uid,
                    id, self._payment_fields(cr, uid, statement_data[2], context=context),
                    context=context)
        return order_ids
    ...
    def create_from_ui(self, cr, uid, orders, context=None):
        ...
        orders_to_save = orders = [o for o in orders if o['data']['name'] not in existing_references]
        ...
        * call the parent create_from_ui() for the remaining orders"""
        ...
        # Save Draft Orders
        self._process_draft_order(cr, uid, draft_orders, context=context)

        # Check order for Advance payments
        for inx, order_to_check in enumerate(orders_to_save):
                #first check in order
                _logger.info("Show lines %s=>%s:%s" % (inx, order_to_check['to_invoice'], order_to_check['data']))
    ...
    def _compute_is_partial_paid(self, cr, uid, ids, name, args, context=None):
        res = {}
        for order in self.browse(cr, uid, ids, context=context):
            res[order.id] = {'is_partial_paid': (order.state == 'draft') and len(order.statement_ids) != 0}
        return res
    ...
    _columns = {
        ...
        'is_partial_paid': fields.function(_compute_is_partial_paid,
                                         type='boolean', string='Is Partially Paid', store=True),
    }
    ...
    def _get_out_picking_type(self, cr, uid, context=None):
        return self.pool.get('ir.model.data').xmlid_to_res_id(
                    cr, uid, 'point_of_sale_bg.picking_type_posout', context=context)
    ...
    def search_read_orders(self, cr, uid, query, context=None):
        condition = [
            ('state', '=', 'draft'),
            ('statement_ids', '=', False),
            '|',
        ...
        return self.search_read(cr, uid, condition, fields, context=context, limit=10)

    def load_order(self, cr, uid, order_id, context=None):
        orderlines = []
        order = self.browse(cr, uid, order_id, context=context)
        _logger.debug("Load order from frontend %s:%s:%s" % (order_id, order, order.lines))
        ids = [l.id for l in order.lines]
        #fields = ['product_id', 'price_unit', 'qty', 'discount']
        #olr = self.pool.get('pos.order.line').read(cr, uid, ids, fields=fields, context=context, load='_classic_read')
        #_logger.debug("lines read %s" % olr)
        ol = self.pool.get('pos.order.line').browse(cr, uid, ids, context=context)
        for l in ol:
...
class pos_order_line(osv.osv):
    ...
    def _compute_taxes(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            taxes = account_tax_obj.compute_all(cr, uid, line.tax_id, price, line.qty, product=line.product_id, partner=line.order_id.partner_id or False)
    ...
    def _amount_line_all(self, cr, uid, ids, field_names, arg, context=None):
        ...
        taxes = self._compute_taxes(self, cr, uid, ids, context=context)
        for line in self.browse(cr, uid, ids, context=context):
