function openerp_pos_db(instance, module){ 
    ...
    module.PosDB = instance.web.Class.extend({
        ...
        init: function(options){
            ...
            this.product_margin_classification = {};
            ...
            this.users_by_id = {};
            // required password
            this.user_required_password = false;
        ...
        set_required_password: function(config) {
                this.user_required_password = config.required_password || false;
        },
        get_required_password: function(config) {
                return this.user_required_password;
        },
        get_user_password: function(user_id) {
                var user = this.get_user(user_id);

                if (_.isEmpty(user)) {
                        return false;
                } else {
                        return user.cashier_password;
                }
        },
        ...
        add_products: function(products){
            ...
            for(var i = 0, len = products.length; i < len; i++){
                ...
                if(product.income_pdt === true){
                    product.type = product.type + 'in';
                }
                if(product.expense_pdt === true){
                    product.type = product.type + 'out';
                }
        ...
        add_margin_classification: function (margins) {
            if (!(margins instanceof Array)) {
                margins = [margin];
            }
            var margin;
            while (margin = margins.pop()) {
                this.product_margin_classification[margin.id] = margin;
            }
        },
