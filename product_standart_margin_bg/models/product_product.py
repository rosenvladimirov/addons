...
class ProductProduct(Model):
    ...
    @api.multi
    @api.depends('list_vprice', 'standard_price')
    def _get_margin(self):
        for product in self:
            #_logger.info("Prices %s:%s" % (product.list_vprice, product.standard_price))
            product.standard_margin =\
                product.list_vprice - product.standard_price
            if product.standard_price != 0.0:
                product.standard_margin_rate = ((product.list_vprice/product.standard_price) -1)*100
            else:
    ...
    standard_margin_rate = fields.Float(
        compute=_get_margin, string='Theorical Margin (%)', store=False,
