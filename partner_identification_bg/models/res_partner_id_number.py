...
class ResPartnerIdNumber(models.Model):
    ...
    name = fields.Char(
        string="ID Number", required=True, index=True,
    ...
    def create(self, vals):
        ...
        if vals.get('name') and vals.get('category_id'):
            ...
            if category and category.validation_model == 'ir.sequence':
                ir_seq_obj = self.env['ir.sequence']
                ir_seq = ir_seq_obj.search([('name','=',"%s_%s" % (category.name, category.code)), ('code', '=', 'res.partner.autoseq')])
                if ir_seq:
                    vals['name'] = ir_seq_obj.get_id(ir_seq.id)
            if category and category.fieldname != '':
