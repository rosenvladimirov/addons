...
VALIDATION_MODELS = [
    ...
    ('us.ssn','SSN (U.S.'),
    ('us.tin','TIN (U.S.'),
    ('ir.sequence', 'Auto sequence number'),
...
class ResPartnerIdCategory(models.Model):
    ...
    def _default_fld(self):
        return {
                'ref': {'code': 'VE', 'name': 'res_partner', 'active': True, 'validation_model': 'ir.sequence', 'validation_model_base': 'ir.sequence', 'fieldname': 'ref'},
    ...
    def _get_fieldname(self):
        return [('ref', 'Contact Reference'), ('vat', 'Inditification code for VAT registrations'), ('company_registry', 'Trade Agency register number'),]
    ...
    validation_model = fields.Selection(VALIDATION_MODELS, 'Integrated validation Models',
                                     required=True, default='eu.vat', index=True)
    ...
    def create(self, vals):
        if 'ir.sequence' and 'code' in vals:
            ir_obj = self.env['ir.sequence']
            if vals['code']:
                ir = ir_obj.search([('name', 'ilike', "%s_%s" % (vals['name'].lower(), vals['code'].lower()))])
                if not ir:
                    ir.create({'name': "%s_%s" % (vals['name'].lower(), vals['code'].lower()), 'padding': 5, 'prefix': "%s" % vals['code'].upper(), 'code': 'res.partner', 'company_id': vals.get('company_id', False)})
            if not vals.get('validation_model_base'):
                vals['validation_model_base'] = vals['validation_model']
        return super(ResPartnerIdCategory, self).create(vals)
    ...
    def _change_validation_model(self):
        if self.validation_model not in ['other','ir.sequence']:
    ...
    def validate_id_number(self, id_number):
        ...
        ret = True
        try:
            if not self.validation_model in ['ir.sequence', 'other']:
