...
class AccountAssetLine(models.Model):
    ...
    @api.model
    def _get_type_ctx(self):
        return self._context.get('mode', 'depreciate')
    ...
    depreciated_value = fields.Float(
        ...
        store=True)
    taxadmin_amount = fields.Float(
        string='Taxadmin Amount', digits=dp.get_precision('Account'),
        required=True)
    taxadmin_remaining_value = fields.Float(
        compute='_compute_taxadmin_values',
        digits=dp.get_precision('Account'),
        string='Next Period Taxadmin Depreciation',
        store=True)
    taxadmin_depreciated_value = fields.Float(
        compute='_compute_taxadmin_values',
        digits=dp.get_precision('Account'),
        string='Amount Already Taxadmin Depreciated',
        store=True)
    ...
    type = fields.Selection(
        ...
        readonly=True, default=_get_type_ctx)
    ...
    @api.depends('taxadmin_amount', 'previous_id')
    @api.multi
    def _compute_taxadmin_values(self):
        dlines = self
        if self._context.get('no_compute_asset_line_ids'):
            # skip compute for lines in unlink
            exclude_ids = self._context['no_compute_asset_line_ids']
            dlines = dlines.filtered(lambda l: l.id not in exclude_ids)
        dlines = self.filtered(lambda l: l.type == 'taxadmin')
        dlines = dlines.sorted(key=lambda l: l.line_date)
        ...
        for i, dl in enumerate(dlines):
            if i == 0:
                depreciation_base = dl.depreciation_base
                depreciated_value = dl.previous_id \
                    and (depreciation_base - dl.previous_id.taxadmin_remaining_value) \
                    or 0.0
                remaining_value = \
                    depreciation_base - depreciated_value - dl.taxadmin_amount
            else:
                depreciated_value += dl.previous_id.taxadmin_amount
                remaining_value -= dl.taxadmin_amount
            dl.taxadmin_depreciated_value = depreciated_value
            dl.taxadmin_remaining_value = remaining_value
    ...
    @api.onchange('amount', 'taxadmin_amount')
    def _onchange_amount(self):
        if self.type == 'depreciate':
            self.remaining_value = self.depreciation_base - \
                self.depreciated_value - self.amount
        if self.type == 'taxadmin':
            self.taxadmin_remaining_value = self.depreciation_base - \
                self.taxadmin_depreciated_value - self.taxadmin_amount
        ...
        for dl in self:
            ...
            if vals.keys() == ['move_id'] and not vals['move_id']:
                ...
                check = asset_lines.filtered(
                    lambda l: l.move_check and l.type in ['depreciate', 'taxadmin'] and
