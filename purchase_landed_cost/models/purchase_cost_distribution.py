...
class PurchaseCostDistribution(models.Model):
    ...
    @api.one
    @api.depends('expense_lines', 'expense_lines.expense_amount_vat')
    def _compute_total_expense_vat(self):
        self.total_expense_vat = sum([x.expense_amount_vat for x in
                                  self.expense_lines])
    ...
    total_expense_vat = fields.Float(
        compute=_compute_total_expense_vat,
        digits_compute=dp.get_precision('Account'), string='Total expenses VAT')
    ...
    def _prepare_expense_line(self, expense_line, cost_line):
        ...
        if divisor:
            expense_amount = (expense_line.expense_amount * multiplier /
                              divisor)
            expense_amount_vat = (expense_line.expense_amount_vat * multiplier /
                              divisor)
        else:
            raise exceptions.Warning(
        ...
        return {
            ...
            'cost_ratio_vat':       expense_amount_vat / cost_line.product_qty,
        }
    ...
    def action_calculate(self):
        for distribution in self:
            ...
            if any([not x.expense_amount for x in distribution.expense_lines]) and \
                any([not x.expense_amount_vat for x in distribution.expense_lines]):
                raise exceptions.Warning(
    ...
    def action_done(self):
        for line in self.cost_lines:
            if self.cost_update_type == 'direct' and line.standard_price_new != line.standard_price_old:
            ...
            if self.cost_update_type == 'direct' and line.standard_price_new_vat != line.standard_price_old_vat:
                #invl_obj = self.env['account.invoice.line']
                linv = line.invoice_line_id
                linv.sudo().write({"price_unit_vat": line.standard_price_new_vat})
                #purchase_id
        self.state = 'done'
...
class PurchaseCostDistributionLine(models.Model):
    ...
    @api.one
    @api.depends('expense_lines', 'expense_lines.cost_ratio', 'expense_lines.cost_ratio_vat')
    def _compute_cost_ratio(self):
        self.cost_ratio = sum([x.cost_ratio for x in self.expense_lines])
        self.cost_ratio_vat = sum([x.cost_ratio_vat for x in self.expense_lines])
    ...
    @api.one
    @api.depends('expense_lines', 'expense_lines.expense_amount_vat')
    def _compute_expense_amount_vat(self):
        self.expense_amount_vat = sum([x.expense_amount_vat for x in
                                   self.expense_lines])

    @api.one
    @api.depends('standard_price_old', 'cost_ratio', 'cost_ratio_vat')
    def _compute_standard_price_new(self):
        self.standard_price_new = self.standard_price_old + self.cost_ratio
        self.standard_price_new_vat = self.standard_price_old_vat + self.cost_ratio_vat
...
#    @api.one
#    @api.depends('move_id', 'move_id.purchase_line_id.order_id')
#    def _get_invoice_id(self):
        #invl_obj = self.env['account.invoice.line']
        #self.invoice_id = invl_obj.search([("purchase_line_id", "=", self.move_id.purchase_line_id.order_id.id)], limit=1).invoice_id.id

    @api.one
    @api.depends('move_id', 'move_id.purchase_line_id.invoice_lines')
    def _get_invoice_line_id(self):
        #invl_obj = self.env['account.invoice.line']
        self.invoice_line_id = [(0, False, [x.id for x in self.move_id.purchase_line_id.invoice_lines])]
        #self.invoice_line_id = invl_obj.search([("purchase_line_id", "=", self.move_id.purchase_line_id.order_id.id)], limit=1).id
    ...
    @api.one
    @api.depends('product_price_unit_vat')
    def _get_standard_price_old_vat(self):
        self.standard_price_old_vat = self.product_price_unit_vat
...
#        comodel_name='account.invoice', string='Invoice ID', readonly=True,
#        compute=_get_invoice_id, delegate=True, store=True)
    invoice_line_id = fields.Many2one(
        comodel_name='account.invoice.line', string='Invoice line', readonly=True,
        compute=_get_invoice_line_id, store=True)
#    invoice_line_id = fields.Many2one(
#        comodel_name='purchase.order.line', string='Invoice Number', readonly=True,
#        related='move_id.purchase_line_id.invoice_lines', store=True)
    ...
    product_price_unit_vat = fields.Float(
        string='Unit price VAT', related='invoice_line_id.price_unit_vat')
    expense_lines = fields.One2many(
    ...
    standard_price_old = fields.Float(
        ...
        digits_compute=dp.get_precision('Product Price'))
    standard_price_old_vat = fields.Float(
        string='Previous cost for VAT price', compute="_get_standard_price_old_vat", store=True,
        digits_compute=dp.get_precision('Product Price'))
    expense_amount = fields.Float(
        string='Cost amount', digits_compute=dp.get_precision('Account'),
        compute='_compute_expense_amount')
    expense_amount_vat = fields.Float(
        string='Cost amount VAT', digits_compute=dp.get_precision('Account'),
        compute='_compute_expense_amount_vat')
    cost_ratio = fields.Float(
        string='Unit cost', compute='_compute_cost_ratio')
    cost_ratio_vat = fields.Float(
        string='Unit cost for VAT price', compute='_compute_cost_ratio')
    ...
    standard_price_new_vat = fields.Float(
        string='New cost VAT', digits_compute=dp.get_precision('Product Price'),
        compute='_compute_standard_price_new')
...
class PurchaseCostDistributionLineExpense(models.Model):
    ...
    expense_amount = fields.Float(
        ...
        digits_compute=dp.get_precision('Account'))
    expense_amount_vat = fields.Float(
        string='Expense amount for VAT', default=0.0,
        digits_compute=dp.get_precision('Account'))
    cost_ratio = fields.Float('Unit cost', default=0.0)
    cost_ratio_vat = fields.Float('Unit cost for VAT price', default=0.0)
    company_id = fields.Many2one(
...
class PurchaseCostDistributionExpense(models.Model):
    ...
    expense_amount = fields.Float(
        ...
        required=True)
    expense_amount_vat = fields.Float(
        string='Expense amount for VAT', digits_compute=dp.get_precision('Account'),
    ...
    @api.one
    @api.depends('distribution', 'type', 'expense_amount', 'expense_amount_vat')
    def _compute_display_name(self):
    ...
    def onchange_invoice_line(self):
        ...
        self.expense_amount = (self.type.vat_expense != True) and self.invoice_line.price_subtotal or 0.0
        self.expense_amount_vat = (self.type.vat_expense == True) and self.invoice_line.price_subtotal or 0.0
