...
class account_voucher(osv.Model):
    ...
    def first_move_line_get(self, cr, uid, voucher_id, move_id, company_currency, current_currency, context=None):
        ...
        if voucher.journal_id.posting_policy == 'storno':
            debit = credit = 0.0
    ...
    def voucher_move_line_create(self, cr, uid, voucher_id, line_total, move_id, company_currency, current_currency, context=None):
        moves = super(account_voucher, self).voucher_move_line_create(cr, uid, voucher_id, line_total, move_id, company_currency, current_currency, context=context)
        voucher = self.pool.get('account.voucher').browse(cr, uid, voucher_id, context)
        ...
        if moves:
            for move in moves[1]:
                ...
                _logger.info("Vaucher line %s:%s" % (inv_line.journal_id.name, voucher_line.journal_id.name))
                if voucher_line.journal_id.posting_policy == 'storno':
                    if voucher_line and inv_line:
                        if inv_line.credit < 0:
                            move_line_obj.write(cr, uid, [voucher_line.id], {'debit': -voucher_line.credit, 'credit': 0.0})
                        if inv_line.debit < 0:
                            move_line_obj.write(cr, uid, [voucher_line.id], {'credit': -voucher_line.debit, 'debit': 0.0})
        return moves
