...
class account_invoice(models.Model):
    ...
    def line_get_convert(self, line, part, date):
        ...
        inv = False
        if self.journal_id:
            is_storno = self.journal_id.posting_policy == 'storno'
            inv_type = self.type
        else:
            inv =  self.env['account.invoice'].browse([line.get('invoice_id')])
            inv_type = inv.type
            if inv and inv.journal_id.posting_policy == 'storno':
                is_storno = True
        res = super(account_invoice, self).line_get_convert(line, part, date)
        ctx = dict(self._context)
        _logger.info("Line convert %s:%s:%s" % (inv and inv.type, inv and inv.journal_id.name or '', line))
        if is_storno:
            credit = debit = 0.0
